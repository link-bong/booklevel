...
import src.main.java.com.novelplatform.ui.dialogs.ChapterManagementDialog;
import javax.swing.*;
...
public class BookManagementPanel extends JPanel {
    ...
    private void initUI() {
        ...
        JButton viewChaptersBtn = new JButton("查看章节");
        ...
        viewChaptersBtn.addActionListener(this::handleViewChapters);

        toolPanel.add(addBtn);
        toolPanel.add(editBtn);
        toolPanel.add(refreshBtn);
        toolPanel.add(viewChaptersBtn);
    ...
    private void handleViewChapters(ActionEvent e) {
        int selectedRow = bookTable.getSelectedRow();
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "请选择一本书查看章节。");
            return;
        }
        String bookName = (String) tableModel.getValueAt(selectedRow, 0);
        int bookId = getBookId(bookName);
        if (bookId != -1) {
            ...
            new ChapterManagementDialog(parentFrame, bookId).setVisible(true);
        }
    }
}